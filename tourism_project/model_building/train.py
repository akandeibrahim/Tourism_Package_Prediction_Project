...
mlflow.set_experiment("mlops-training-experiment")     # complete the code to set the MLflow experiment name (same as the dev experimentation cell)
...
numeric_features = ['Age', 'CityTier', 'DurationOfPitch', 'NumberOfPersonVisiting', 'NumberOfFollowups',
                    ...
                    'NumberOfChildrenVisiting', 'MonthlyIncome']   # complete the code to list all numerical feature names (same as in prep.py)

categorical_features = ['TypeofContact', 'Occupation', 'Gender',
                        'ProductPitched', 'MaritalStatus', 'Designation']   # complete the code to list all categorical feature names (same as in prep.py)
...
param_grid = {
    ...
    'xgbclassifier__learning_rate': [0.01, 0.05],       # Step size used during boosting. Smaller values may improve generalization but require more trees.
    'xgbclassifier__reg_lambda': [0.1, 1],          # L2 regularization strength. Higher values help reduce overfitting.
}
# Model pipeline
...
with mlflow.start_run():
    ...
    model_path = "tourism_project/deployment/"   # Specify the local file path (inside tourism_project/deployment/) where the trained model should be saved.
